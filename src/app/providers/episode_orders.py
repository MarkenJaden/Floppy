...
def list_orders(provider, series_id, *, user=None, language=None):
    ...
    if provider == "tmdb":
        ...
        data = response["data"]
        types = data.get("seasonTypes") or [row["type"] for row in data["seasons"]]
...
def _episode(row, provider, season=None, number=None):
    season = (
        row.get("season_number", row.get("seasonNumber")) if season is None else season
    )
...
def fetch_order(provider, series_id, order_key, *, user=None, language=None):
    ...
    if provider == "tvdb":
        ...
        for season in seasons:
            ...
            response = _request(
                provider, f"tv/{series_id}/season/{number}", user, language
            )
