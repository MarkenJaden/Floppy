...
def persist_order(show, provider, series_id, key, label, catalogue):
    ...
    order, _ = EpisodeOrder.objects.get_or_create(
        ...
        key=key,
        revision=_digest(catalogue),
        defaults={"label": label, "catalogue": catalogue},
    )
    return order
...
def metadata_for_order(media_type, order, season_numbers=None, episode_number=None):
        ...
        episodes = [
            {
                **row,
                ...
                "name": row["title"],
                "still_path": row.get("image"),
                "vote_average": 0,
                "vote_count": 0,
            }
            for row in rows
            if row["season_number"] == number
        ]
        season = {
            **base,
            "media_type": "season",
    ...
    base["details"] = {
        "seasons": len(base["related"]["seasons"]),
        "episodes": len(rows),
    }
    if media_type == "episode":
        number = (
            season_numbers[0]
            if isinstance(season_numbers, (list, tuple))
            else season_numbers
        )
        row = next(
            (
...
def _watches(tv, *, lock=False):
    rows = Episode.objects.filter(
        related_season__related_tv=tv, related_season__order_archived=False
    ).order_by("pk")
...
def _state(tv, order, rows):
    return {
        "tv_id": tv.pk,
        "active_order": tv.active_episode_order_id,
        "order_id": order.pk,
        ...
        "watches": [
            {
                ...
                "watch_operation_id": row.watch_operation_id,
                **{field: getattr(row, field) for field in WATCH_FIELDS},
            }
            for row in rows
        ],
    }
...
def preview_change(tv, order):
    ...
    for row in rows:
        proven = (
            row.item.source == order.provider
            and row.item.provider_episode_id in identities
        )
        mappings.append(
            {
                "watch_ids": [row.pk],
                "episode_ids": [row.item.provider_episode_id] if proven else [],
                "archive": False,
            }
        )
    return {
        "token": _digest(_state(tv, order, rows)),
...
def apply_change(tv, order, *, token, resolutions):
    ...
    for resolution in resolutions:
        ...
        if (
            ...
            or seen.intersection(sources)
            or not set(sources) <= by_id.keys()
        ):
        ...
        if (
            ...
            or not set(targets) <= catalogue.keys()
        ):
            raise ValueError(
                "Select destination episode identities or explicitly archive"
            )
        ...
        for field in WATCH_FIELDS:
            if (
                len({str(getattr(by_id[source], field)) for source in sources}) > 1
    ...
    for resolution in resolutions:
        ...
        for index, identity in enumerate(resolution["episode_ids"]):
            ...
            if season is None:
                season = Season(
                    related_tv=tv, item=season_item, user=tv.user, status=tv.status
                )
